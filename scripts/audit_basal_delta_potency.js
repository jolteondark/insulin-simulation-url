#!/usr/bin/env node
'use strict';
// Triggered mechanism audit; no outcome calibration.
...
const md=['# Basal dose-deviation potency audit','','This is a unit-consistency mechanism audit, not outcome calibration. The maintenance basal requirement remains implicit in the frozen fasting equilibrium; only deviations from the reference basal dose are challenged. Unit-consistent gain is derived from the simulator prandial 1-U integrated effect.','','Derived unit-consistent daily gain: '+out.derived_daily_gain.toFixed(2)+' mg/dL-equivalent per U at SI=1.','','| mode | basal multiplier | mean glucose | TBR <54 | patients nocturnal <54 |','|---|---:|---:|---:|---:|'];for(const r of rows)md.push(`| ${r.mode} | ${r.mult.toFixed(1)} | ${r.mean.toFixed(1)} | ${r.tbr54.toFixed(2)}% | ${r.nocturnal54.toFixed(1)}% |`);md.push('','## Checks');for(const [n,ok] of checks)md.push(`- ${ok?'PASS':'FAIL'} — ${n}`);md.push('','## Guardrails','- Do not fit the derived gain to Emory glucose metrics.','- Existing simulations remain unchanged unless the optional basal_delta_gain_per_day hook is enabled.','- This audit does not establish the final U100/U300 time profile; it only tests missing per-unit potency of basal-dose deviations.');fs.writeFileSync(dir+'/report.md',md.join('\n')+'\n');console.log(md.join('\n'));if(checks.some(x=>!x[1]))process.exitCode=2;
