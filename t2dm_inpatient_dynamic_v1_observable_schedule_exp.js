...
function normalizeMinute(x,fallback){const n=Math.round(Number(x));return Number.isFinite(n)?Math.max(0,Math.min(1439,n)):fallback}
function simulateDay(baseModel,p,order,state={},seed=1,prevState=null){
 ...
 const bedtimeCorrectionMin=normalizeMinute(state.bedtime_correction_min??1260,1260);
 const pocMinByMeal={
  breakfast:normalizeMinute(state.poc_min_by_meal&&state.poc_min_by_meal.breakfast,420),
  lunch:normalizeMinute(state.poc_min_by_meal&&state.poc_min_by_meal.lunch,720),
  dinner:normalizeMinute(state.poc_min_by_meal&&state.poc_min_by_meal.dinner,1080)
 };
 const observableCorrectionSchedule=[...new Set((Array.isArray(state.observable_correction_schedule_min)?state.observable_correction_schedule_min:[]).map(x=>normalizeMinute(x,-1)).filter(x=>x>=0))].sort((a,b)=>a-b);
 const observableCorrectionSet=new Set(observableCorrectionSchedule);
 ...
 const mealBolusMeta=[{meal:'breakfast',poc_min:pocMinByMeal.breakfast,index:0,adjusted:false},{meal:'lunch',poc_min:pocMinByMeal.lunch,index:1,adjusted:false},{meal:'dinner',poc_min:pocMinByMeal.dinner,index:2,adjusted:false}];
 const observedPoc={},prandialSafetyAdjustments=[];
 const bedtimeCorrections=[],observableCorrections=[],hypoglycemiaRescues=[],rescueCarbEvents=[];
 ...
 function stressAt(t){
  if(Array.isArray(state.stress_blocks)){let s=0;for(const b of state.stress_blocks){if(t>=b.start_min&&t<b.end_min)s=Math.max(s,Number(b.severity)||0)}return clamp(s,0,1)}
 ...
 for(let t=0;t<n-1;t++){
  ...
  if(prandialSafetyAdjustmentFn){
   for(const meta of mealBolusMeta){
    ...
    if(Number.isFinite(given)){b[1]=given;if(given!==planned)prandialSafetyAdjustments.push({meal:meta.meal,poc_min:meta.poc_min,poc_glucose_mg_dl:poc,bolus_min:t,planned_units:planned,given_units:given});}
   }
  }
  if(observableCorrectionFn&&observableCorrectionSet.has(t)){
   const raw=observableCorrectionFn({t,glucose_mg_dl:g[t],patient:p,order_u:dose,si_relative:si,effective_insulin_sensitivity:effectiveInsulinSensitivity,state,prior_corrections:observableCorrections.slice()});
   let u=0,label='observable_correction';
   if(Number.isFinite(Number(raw)))u=Math.max(0,Math.round(Number(raw)||0));
   else if(raw&&typeof raw==='object'){u=Math.max(0,Math.round(Number(raw.units)||0));if(raw.label)label=String(raw.label)}
   if(u>0){bolus.push([t,u]);observableCorrections.push({minute:t,glucose_mg_dl:g[t],units:u,label});}
  }
  ...
  if(hypoglycemiaRescueFn&&t>=nextRescueAllowedMin){
   ...
   else if(raw&&typeof raw==='object'){carbs=Math.max(0,Number(raw.carbs_g)||0);cooldown=Math.max(1,Math.round(Number(raw.cooldown_min)||15));if(raw.label)label=String(raw.label)}
