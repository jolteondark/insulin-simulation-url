(function(root){
  ...
  function integrate(){
    ...
    if(details)glance.insertBefore(feedback,details); else glance.appendChild(feedback);
    feedback.classList.add('result-glance-feedback');
    return true;
  }
  function schedule(){setTimeout(integrate,0);}
  function mount(){
    ...
    document.addEventListener('click',event=>{if(event?.target?.id==='submitBtn')schedule();});
    integrate();
  }
