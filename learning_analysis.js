(function(){
  ...
  function feedbackActionMetrics(traces){let eligible=0,aligned=0;
    for(const t of traces){const ds=t.days||[];for(let i=0;i<ds.length-1;i++){const d=ds[i],next=ds[i+1],spec=FEEDBACK_ACTIONS[d?.feedback?.primary_tag];if(!spec)continue;
      if(spec.source==='scheduled'&&Number(d?.correction_doses_u?.[spec.correctionKey]||0)>0)continue;
  ...
  function traceMetrics(traces){let correction=0,rapidDelivered=0,repeatPairs=0,repeatSame=0;
    ...
    return {correction_share_of_rapid:ratio(correction,rapidDelivered),same_feedback_next_day_rate:ratio(repeatSame,repeatPairs),trace_days:traces.reduce((n,t)=>n+(t.days?.length||0),0),...feedbackActionMetrics(traces)};
  }
  ...
  window.WardLearningAnalysis={normalize,completedCases,splitTerciles,groupMetrics,traceMetrics,feedbackActionMetrics,summarize,renderHtml,refresh,METRICS,MIN_CASES,FEEDBACK_ACTIONS,version:'1.4.0'};
