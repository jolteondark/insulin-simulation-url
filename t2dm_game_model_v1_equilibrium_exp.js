...
const SCALE={meal_gain:.025,bolus_gain:.24,basal_delta_gain:.30,restore_gain:.008};
...
function generatePatient(seed=1){
 ...
 return{patient:p,case:{case_id:'T2V1-'+String(seed),phenotype:'T2DM',meal_plan_carb_g:{...DEFAULT_MEALS},model_version:'t2dm-v1-equilibrium-exp-2026-08-20'},state:null,history:[]};
}
...
function simulateDay(p,order,ctx={},seed=1,state=null){
 ...
 let mn=g[0],mx=g[0];
 for(let t=0;t<n-1;t++){
   let mealDrive=0;for(const [tm,c] of mealEvents){const dt=t-tm;if(dt>=0&&dt<240)mealDrive+=c*(dt/55)*Math.exp(1-dt/55)*SCALE.meal_gain*mealResponse;}
   let bolusDrive=0;for(const [tb,u] of bolus){const dt=t-tb;if(dt>=0&&dt<300)bolusDrive+=u*(dt/70)*Math.exp(1-dt/70)*SCALE.bolus_gain*p.si_relative;}
...
window.T2DMGameModelV1EquilibriumExp={version:'0.1-equilibrium-exp-2026-08-20',SCALE,MEAL_RESPONSE_SCALE,generatePatient,suggestOrder,simulateDay,playDay,mealResponseMultiplier,roundUnit};
})();