...
const out={protocol:{N,days:1,meal_cv:.20,timing:'freeze default'},strategies:{}};
for(const name of Object.keys(strategies))out.strategies[name]={survive:0,low:0,high:0,min:[],max:[]};
let priorUnsafe=0;
for(let i=0;i<N;i++){
  const seed=100000+i*7919;
  const base=T1DMGameModelV2.generatePatient(seed);
  const starter={...base.case.previous_order_u};
  const prior=T1DMGameModelV2.playDay(base,starter,{intake_fraction:{breakfast:1,lunch:1,dinner:1},meal_timing_sd_min:0,bolus_timing_sd_min:0,poc_timing_sd_min:0},seed+1);
  ...
  for(const [name,fn] of Object.entries(strategies)){
    ...
    T1DMGameModelV2.playDay(game,starter,{intake_fraction:{breakfast:1,lunch:1,dinner:1},meal_timing_sd_min:0,bolus_timing_sd_min:0,poc_timing_sd_min:0},seed+1);
