...
const TRANSIENT_CONFIG={si_sigma:.22,beta_sigma:.15,intake_sigma:.20,missed_meal_probability:.05,missed_meal_fraction:.40,intake_min:.15,intake_max:1.40};
...
function generatePatient(seed=1){
 ...
 return{patient:p,case:{case_id:'T2-'+String(seed),phenotype:'T2DM',meal_plan_carb_g:{...DEFAULT_MEALS},model_version:'t2dm-v0-transient-exp-2026-08-20'},state:null,history:[]};
}
...
function sampleDailyTransient(seed,cfg=TRANSIENT_CONFIG){
 const si_multiplier=Math.exp(cfg.si_sigma*randn('t2-day-si:'+seed)-.5*cfg.si_sigma*cfg.si_sigma);
 const beta_multiplier=Math.exp(cfg.beta_sigma*randn('t2-day-beta:'+seed)-.5*cfg.beta_sigma*cfg.beta_sigma);
 function intake(label){
   let x=1+cfg.intake_sigma*randn('t2-day-intake-'+label+':'+seed);
   if(rng('t2-day-miss-'+label+':'+seed)()<cfg.missed_meal_probability)x*=cfg.missed_meal_fraction;
   return clamp(x,cfg.intake_min,cfg.intake_max);
 }
 return{si_multiplier,beta_multiplier,intake_fraction:{breakfast:intake('breakfast'),lunch:intake('lunch'),dinner:intake('dinner')}};
}
function simulateDay(p,order,ctx={},seed=1,state=null){
 const n=1441,g=new Float64Array(n),meal={...DEFAULT_MEALS,...(ctx.meal_plan_carb_g||{})};
 const transient=ctx.transient||sampleDailyTransient(seed,ctx.transient_config||TRANSIENT_CONFIG);
 const intake={...transient.intake_fraction,...(ctx.intake_fraction||{})};
 ...
 const mealResponse=mealResponseMultiplier(p),siEff=p.si_relative*transient.si_multiplier;
 let mn=g[0],mx=g[0];
 for(let t=0;t<n-1;t++){
   ...
   let bolusDrive=0;for(const [tb,u] of bolus){const dt=t-tb;if(dt>=0&&dt<300)bolusDrive+=u*(dt/70)*Math.exp(1-dt/70)*PROVISIONAL_SCALE.bolus_gain*siEff;}
   const basalDrive=dose.basal_u/1440*PROVISIONAL_SCALE.basal_gain*siEff;
   const endo=betaInsulinUeqPerMin(p,g[t])*transient.beta_multiplier*PROVISIONAL_SCALE.endogenous_gain*siEff;
 ...
 return{bg,min:mn,max:mx,end:g[1440],series:g,order_u:dose,actual_meal_carb_g:{breakfast:meal.breakfast*intake.breakfast,lunch:meal.lunch*intake.lunch,dinner:meal.dinner*intake.dinner},next_state:{glucose_mg_dl:g[1440]},meal_response_multiplier:mealResponse,transient};
}
function playDay(game,order,ctx={},seed=1){const r=simulateDay(game.patient,order,ctx,seed,game.state);game.state=r.next_state;game.history.push({day:game.history.length+1,order_u:r.order_u,meal_carb_g:r.actual_meal_carb_g,bg:r.bg,min:r.min,max:r.max,transient:r.transient});game.case.previous_order_u=r.order_u;game.case.previous_day_4point_bg_mg_dl=r.bg;return r;}
window.T2DMGameModelV0TransientExp={version:'0.1-transient-exp-2026-08-20',PROVISIONAL_SCALE,MEAL_RESPONSE_SCALE,TRANSIENT_CONFIG,generatePatient,suggestOrder,simulateDay,playDay,betaInsulinUeqPerMin,mealResponseMultiplier,sampleDailyTransient,roundUnit};
})();