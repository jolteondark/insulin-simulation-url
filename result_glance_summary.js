(function(root){
  ...
  function doseExcursionLink(x,doseKey,primaryKey=''){const bgKey=DOSE_TO_BG[doseKey];if(!bgKey)return null;const value=x?.bg?.[bgKey];const status=bgStatus(value);if(!status.label)return null;return {doseKey,bgKey,bgLabel:BG_LABELS[bgKey],value,status:status.label.toLowerCase(),label:status.label,primary:doseKey===primaryKey}}
  function linkMarkup(link){if(!link)return '';const prefix=link.primary?'← 次の1点・':'← ';return `<span class="result-glance-dose-link">${prefix}${link.bgLabel}${link.label}</span>`}
  function actualDoseChip(label,value,extra,link){const scale=extra>0?`<span class="result-glance-scale">+scale ${fmt(extra)}</span>`:'';const linkedClass=link?` result-glance-dose-linked result-glance-dose-linked-${link.status}${link.primary?' result-glance-dose-primary':''}`:'';return `<span class="result-glance-dose-chip${linkedClass}"><span>${label}</span><b>${fmt(value)} U</b>${scale}${linkMarkup(link)}</span>`}
  ...
  function buildHtml(rec){
    ...
    const day=Number(rec?.day);const title=Number.isFinite(day)?`DAY ${day}：結果 → 次の1点`:'結果 → 次の1点';const hidden=hiddenStatus(x.hidden);const primaryKey=primaryDoseKey(rec);
    return `<div class="result-glance" aria-label="本日の血糖結果と実投与の要約">
      ...
      <div class="result-glance-actual" aria-label="本日の実投与インスリン。血糖逸脱時は対応する直前投与を表示し、教育feedbackのprimary targetを次の1点として強調"><span class="result-glance-actual-label">実投与</span>${actualDoseChip('朝',x.actual.breakfast,x.correction.breakfast,doseExcursionLink(x,'breakfast',primaryKey))}${actualDoseChip('昼',x.actual.lunch,x.correction.lunch,doseExcursionLink(x,'lunch',primaryKey))}${actualDoseChip('夕',x.actual.dinner,x.correction.dinner,doseExcursionLink(x,'dinner',primaryKey))}${basalDoseChip(x.actual.basal,x.scheduled.basal,doseExcursionLink(x,'basal',primaryKey))}</div>
  ...
  function ensureStyle(){
    ...
    const style=document.createElement('style');style.id='resultGlanceStyle';
